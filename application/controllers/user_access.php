...
class User_Access extends CI_Controller
{
    ...
    public function addUserAccessUpdate()
    {
        ...
        $sample = array();
        $aed_send = json_decode(stripslashes($_POST['aed_send']));

        $i = 0;
        foreach ($aed_send as $val) {
            $sample[$i] = $val;
            $i++;
        }
        $i = 0;
    ...
    public function getUserList(){
        ...
        if ($t == 3) {
            ...
            foreach ($info->result_array() as $val) {
                ...
                $post["ware"] = $this->common_model->anyName('ware', 'id', $val["ware"], 'name');
                $post["ware2"] = $val["ware"];
                $post["type"] = $val["type"];
            ...
            foreach ($info->result_array() as $val) {
                $post = array();
                $post["name"] = $this->common_model->anyName('sub_menu', 'id', $val["head"], 'name') . " -> " . $this->common_model->anyName('sub_menu', 'id', $val["sub"], 'name');
    ...
    public function addUserAccess()
    {
        ...
        if (!empty($check)) {
            ...
            $chec = $this->common_model->anyName('ware', 'ch', $shop, 'id');
