...
?>		
	...
	<div class="col-sm-12" style='border-bottom:2px solid'>
				...
				$start_date=date('d.m.Y');
					...
					echo date('d.m.Y',strtotime($start . ' - 1 day'));
...
<?php 
	...
	<td style="text-align:right;font-weight:bold;">
			...
			<a target="_blank" href="<?php echo base_url(); ?>mains/report_ledger/204/2/<?php echo $start ?>/<?php echo $end ?>">
			<?php
...
echo number_format(round($vals[1],2), 2, '.', ',');	
	...
	<a target="_blank" href="<?php echo base_url(); ?>mains/report_ledger/382/2/<?php echo $start ?>/<?php echo $end ?>">
			<div class="col-sm-12">
...
<?php 
		...
		$val=explode(':',$str->openingStock_op(83,$start,$end));


          //   echo "Closing Stock ".$val[2]."    ".$val[4]."     ".$val[5];
		...
		$expense=explode(':',$cgs->getFactory_expense_between(99,$start,$end,$start_b,$end_b));
		
		$cost=$c+$expense[0];
		$cost_b=($c_b+$expense[1]);
  ...
  ?>						
	...
	echo number_format(round($to_b,2), 2, '.', ',');				
				...
				<tr>
					...
					<a target="_blank" href="<?php echo base_url(); ?>mains/trial_balance/<?php echo $start ?>/<?php echo $end; ?>">Less: Administrative & Other Expenses</a>
				...
				<td></td>
					...
					<div class="col-sm-12">
			<?php		
		$expense=explode(':',$cgs->getFactory_expense_between(3,$start,$end,$start,$end));
...
$debit=$debit+$ci->report_model->getBalance3('product_trans','',$start,$end,'dr',$leg['id'],$admin);
					
$credit=$credit+$ci->report_model->getBalance3('product_trans','',$start,$end,'cr',$leg['id'],$admin);
$debit_b=$debit_b+$ci->report_model->getBalance('product_trans','',$start,$end,'dr',$leg['id'],$admin);
					
$credit_b=$credit_b+$ci->report_model->getBalance('product_trans','',$start,$end,'cr',$leg['id'],$admin);
...
$amount_b=$amount_b+$leg['opening_balance'];
			...
			$total_asset=$total_asset+($credit-$debit);
